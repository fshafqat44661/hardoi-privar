...
import { contactStatusSchema } from '@/lib/validators/schemas';
...
export async function PATCH(req: NextRequest, { params }: Params) {
  ...
  try {
    ...
    await connectDB();
    const doc = await ContactSubmission.findByIdAndUpdate(id, { status: parsed.data.status }, { new: true }).lean();
    if (!doc) return fail('Submission not found', 404);
    return ok(mapContactSubmission(doc as never));
  } catch {
